...
FROM (mark INNER JOIN student ON mark.StuID = student.StuID)
         INNER JOIN sub ON mark.SubID = sub.SubID
...
SELECT stu.StuID,
       ...
       stu.StuStatus,
       stu.ClassID,
       AVG(Score) AS 'Diem trung binh'
FROM student stu
         INNER JOIN mark m ON stu.StuID = m.StuID
GROUP BY m.StuID
